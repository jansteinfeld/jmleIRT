...
The `jmleIRT` package includes **Joint Maximum Likelihood Estimation (JMLE)**  for the Rasch model (1PL). It incorporates some selected **bias correction techniques** and offers **Warm's Weighted Likelihood Estimates (WLE)** for person ability estimation.
...
The Joint Maximum Likelihood Estimation (JMLE) for the Rasch model is known to produce biased parameter estimates, especially in finite samples and with extreme response patterns.
The `jmleIRT` package implements two key bias correction techniques:
...
Contributions welcome via GitHub: [jansteinfeld/jmleIRT](https://github.com/jansteinfeld/jmleIRT).
