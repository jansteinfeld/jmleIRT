...
For person $`p = 1, \ldots, P`$ and item $`i = 1, \ldots, I`$, the Rasch
model posits the probability of a correct response $`X_{pi}`$ as:

``` math
P(X_{pi} = 1 \mid \theta_p, \beta_i) = \frac{\exp(\theta_p - \beta_i)}{1 + \exp(\theta_p - \beta_i)}
```

where $`\theta_p`$ represents the ability of person $`p`$ and
...
JMLE alternates updates for $`\theta_p`$ and $`\beta_i`$ until
