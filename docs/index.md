...
- Joint Maximum Likelihood Estimation of person abilities $`\theta_p`$
  and item difficulties $`\beta_i`$.
...
for dichotomous item data. The probability that person $`p`$ with
ability $`\theta_p`$ answers item $`i`$ with difficulty $`\beta_i`$
...
$`\beta = (\beta_1, \ldots, \beta_I)`$ by maximizing the joint
...
$`\theta_p`$ and $`\beta_i`$, until convergence criteria are met.
...
- **Epsilon adjustment:** Proposed by Bertoli Barsotti, Punzo, and
  Lando, this method adds a small positive constant
  ``` math
  \varepsilon > 0
  ```
