# jmleIRT: Rasch Model Joint Maximum Likelihood Estimation in R ![](reference/figures/jmleIRT.png)
...
- Joint Maximum Likelihood Estimation of person abilities $\theta_{p}$
  and item difficulties $\beta_{i}$.
...
$\theta_{p}$ answers item $i$ with difficulty $\beta_{i}$ correctly is
modeled as:

$$P\left( X_{pi} = 1 \mid \theta_{p},\beta_{i} \right) = \frac{e^{\theta_{p} - \beta_{i}}}{1 + e^{\theta_{p} - \beta_{i}}}$$
...
$\theta = \left( \theta_{1},\ldots,\theta_{P} \right)$ and item
difficulties $\beta = \left( \beta_{1},\ldots,\beta_{I} \right)$ by
maximizing the joint likelihood of observed responses:

$$L(\theta,\beta) = \prod\limits_{p = 1}^{P}\prod\limits_{i = 1}^{I}P\left( X_{pi} \mid \theta_{p},\beta_{i} \right)$$

This is typically solved via an iterative algorithm updating
$\theta_{p}$ and $\beta_{i}$, until convergence criteria are met.
...
- **Epsilon adjustment:** Proposed by Bertoli Barsotti, Punzo, and
  ...
  controlled by the argument `eps` in
  [`jmle_estimation()`](jansteinfeld.github.io/jmleIRT/reference/jmle_estimation.md).
  When `eps > 0`, a small positive constant is used to regularize
  extreme response patterns; when `eps = 0` (default), persons with all
  0 or all 1 responses are excluded from the iterative updates and
  assigned ±Inf afterward.

- **Simple post-hoc bias correction:** The option
  `bias_correction = TRUE` in
  [`jmle_estimation()`](jansteinfeld.github.io/jmleIRT/reference/jmle_estimation.md)
  applies a simple finite-sample scaling of item difficulties (factor (I
  − 1)/I) as a fast bias-reduction heuristic.

- **Analytical bias correction:** The function
...
- **`"analytic"`**: A first-order analytical bias correction based on
  ...
  the JMLE solution and then calls
  [`biasCorrection()`](jansteinfeld.github.io/jmleIRT/reference/biasCorrection.md)
  to obtain corrected parameters. The result object contains both the
  raw JMLE (`theta`, `beta`) and the analytically corrected estimates
...
By default,
[`jmle_estimation()`](jansteinfeld.github.io/jmleIRT/reference/jmle_estimation.md)
returns joint ML person estimates `theta`. For reporting individual
abilities, **Warm’s Weighted Likelihood Estimates (WLE)** often have
better bias properties.

- Set `estimatewle = TRUE` in
  [`jmle_estimation()`](jansteinfeld.github.io/jmleIRT/reference/jmle_estimation.md)
  to obtain WLEs in `fit$wle_estimate`.
...
[`jmle_estimation()`](jansteinfeld.github.io/jmleIRT/reference/jmle_estimation.md)
also contains approximate standard errors for person and item parameters
(`se_theta`, `se_beta`), computed from the observed information at the
final JMLE solution.
