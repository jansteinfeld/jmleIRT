...
#'
#' The JML estimators are well-known to suffer from incidental parameter bias when the number of items (I)
#' is finite, resulting in biased parameter estimates. This bias is generally of order \(1/I\).
#'
#' The bias correction implemented here follows the first-order analytic correction approach,
#' which approximates the bias term \(B\) for each parameter as:
#' \deqn{
#' \hat{\theta}^{corr} = \hat{\theta} - \frac{1}{I} \hat{B}_{\theta}, \quad
#' \hat{\beta}^{corr} = \hat{\beta} - \frac{1}{I} \hat{B}_{\beta},
#' }
#' where \(\hat{B}\) is estimated based on the observed scores \(u_i\) and the expected Fisher information \(v_i\),
#' calculated as
#' \deqn{
...
#' where \(p_{ni} = \frac{e^{\theta_n - \beta_i}}{1 + e^{\theta_n - \beta_i}}\) is the probability that person \(n\)
#' correctly answers item \(i\).
#'
#' Missing responses (NA) in the data matrix \(X\) are handled by omitting those entries.
#'
#' @param theta Numeric vector of estimated person parameters \(\hat{\theta}\).
#' @param beta Numeric vector of estimated item parameters \(\hat{\beta}\).
...
#' }
#' for person \(n\) and item \(i\), and then aggregating these across items or persons.
#'
#' The bias terms for person \(n\) and item \(i\) are estimated as:
#' \deqn{
#' \hat{B}_{\theta,n} = \frac{\sum_i v_{ni} u_{ni}}{\sum_i v_{ni}^2}, \quad
#' \hat{B}_{\beta,i} = \frac{\sum_n v_{ni} (p_{ni} - X_{ni})}{\sum_n v_{ni}^2}.
#' }
#'
#' This method is applicable when the number of items \(I\) is fixed and moderate, and
#' the number of persons \(N\) is large.
#'
#' @references
...
biasCorrection <- function(theta, beta, X, I) {
  ...
  res <- biasCorrectionJMLE_cpp(theta = theta, beta = beta, X = X, N = N, I = I)
