 ...
 * theta_corrected = theta - B_theta / T
 * beta_corrected = beta - B_beta / T
 * where T is the number of items.
 *
 ...
 * @param T Integer, number of items (used for scaling bias correction)
 */

void biasCorrectionJMLE_cpp(NumericVector &theta, NumericVector &beta, const NumericMatrix &X,
                            int N, int I, int T)
{
  ...
  // Apply bias correction scaled by 1/T (number of items)
  for (int n = 0; n < N; ++n)
  {
    theta[n] -= biasTheta[n] / static_cast<double>(T);
  }
  for (int i = 0; i < I; ++i)
  {
    beta[i] -= biasBeta[i] / static_cast<double>(T);
  }
}

// [[Rcpp::export]]
void biasCorrectionJMLE(NumericVector &theta, NumericVector &beta,
                        const NumericMatrix &X, int N, int I, int T)
{
  biasCorrectionJMLE_cpp(theta, beta, X, N, I, T);
}
