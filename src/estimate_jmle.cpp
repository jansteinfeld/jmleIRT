#include <Rcpp.h>
#include <cmath>
#include <algorithm>
using namespace Rcpp;

/* -----------------------------------------------------------------------
Helper: numerically stable logistic (sigmoid)
------------------------------------------------------------------------- */
inline double logistic(double x)
{
  if (x >= 0)
  {
    double z = std::exp(-x);
    return 1.0 / (1.0 + z);
  }
  else
  {
    double z = std::exp(x);
    return z / (1.0 + z);
  }
}

/* -----------------------------------------------------------------------
Helper: validate X is a (logical/integer/double) matrix, coerce to NumericMatrix
------------------------------------------------------------------------- */
inline Rcpp::NumericMatrix require_real_matrix(SEXP X_)
{
  if (!Rf_isMatrix(X_))
    Rcpp::stop("X must be a matrix.");
  int t = TYPEOF(X_);
  if (!(t == REALSXP || t == INTSXP || t == LGLSXP))
    Rcpp::stop("X must be a numeric matrix.");
  Rcpp::NumericMatrix X = Rcpp::as<Rcpp::NumericMatrix>(X_);
  return X;
}

/* -----------------------------------------------------------------------
1) WLE estimation
------------------------------------------------------------------------- */
Rcpp::List estimate_wle(NumericMatrix X, NumericVector beta,
                        int max_iter = 1000, double tol = 1e-10,
                        double lower_ext = NA_REAL, double upper_ext = NA_REAL,
                        double wle_adj = 1e-8)
{
  const int N = X.nrow();
  const int K = X.ncol();
  if (K == 0)
    stop("Number of items must be > 0.");
  if (beta.size() != K)
    stop("Length of 'beta' must equal number of columns in 'X'.");

  // Sanity: only 0/1/NA
  for (int i = 0; i < N; ++i)
  {
    for (int k = 0; k < K; ++k)
    {
      double v = X(i, k);
      if (NumericMatrix::is_na(v))
        continue;
      if (!(v == 0.0 || v == 1.0))
        Rcpp::stop("X must contain only 0, 1, or NA (found %.8g at row %d, col %d).",
                   v, i + 1, k + 1);
    }
  }

  NumericVector raw_score(N), wle(N), se(N);
  IntegerVector conv(N), n_iter(N);

  // Main loop over persons calculating raw scores and WLE
  for (int i = 0; i < N; i++)
  {
    int score = 0, J = 0;
    for (int k = 0; k < K; k++)
    {
      double x = X(i, k);
      if (!NumericMatrix::is_na(x))
      {
        score += (int)x;
        J += 1;
      }
    }
    raw_score[i] = score;

    if (J == 0)
    {
      wle[i] = NA_REAL;
      se[i] = NA_REAL;
      conv[i] = 0;
      n_iter[i] = 0;
      continue;
    }

    bool is_extreme = (score == 0 || score == J);
    double score_d = static_cast<double>(score);
    double J_d = static_cast<double>(J);
    double theta = std::log((score_d + 0.5) / (J_d - score_d + 0.5));
    double xstar_const = NA_REAL;

    if (is_extreme)
    {
      double a = std::min(std::max(wle_adj, 1e-8), J_d - 1e-8);
      double target_sum = (score == 0) ? a : (J_d - a);
      xstar_const = target_sum / J_d;
    }

    int converged = 0;
    double fi = 0.0, dfi = 0.0, delta = 0.0;
    int iter = 0;

    for (iter = 0; iter < max_iter; iter++)
    {
      fi = 0.0;
      dfi = 0.0;
      double wle_bias_sum = 0.0;

      for (int k = 0; k < K; k++)
      {
        double x = X(i, k);
        if (NumericMatrix::is_na(x))
          continue;
        double z = theta - beta[k];
        double p = logistic(z);
        double var = p * (1.0 - p);
        double xeff = is_extreme ? xstar_const : x;
        fi += (xeff - p);
        dfi += var;
        wle_bias_sum += var * (1.0 - 2.0 * p);
      }

      if (std::abs(dfi) < 1e-12)
        break;

      double bias = 0.5 * wle_bias_sum / (-dfi);
      double fi_wle = fi - bias;
      delta = fi_wle / (-dfi);
      if (!R_finite(delta))
        break;
      if (std::abs(delta) > 5.0)
        delta = (delta > 0 ? 5.0 : -5.0);

      double theta_new = theta - delta;

      if (std::abs(delta) < tol)
      {
        theta = theta_new;
        converged = 1;
        break;
      }
      theta = theta_new;
    }

    double I_final = 0.0;
    if (converged)
    {
      for (int k = 0; k < K; k++)
      {
        double x = X(i, k);
        if (NumericMatrix::is_na(x))
          continue;
        double z = theta - beta[k];
        double p = logistic(z);
        I_final += p * (1.0 - p);
      }
    }

    wle[i] = theta;
    se[i] = converged ? std::sqrt(1.0 / std::max(I_final, 1e-12)) : NA_REAL;
    conv[i] = converged;
    n_iter[i] = converged ? (iter + 1) : max_iter;
  }

  return List::create(
      Named("raw_score") = raw_score,
      Named("wle") = wle,
      Named("standard_error") = se,
      Named("conv") = conv,
      Named("iterations") = n_iter);
}

/* -----------------------------------------------------------------------
2) JML estimation - CORRECTED
------------------------------------------------------------------------- */
Rcpp::List estimate_jmle(NumericMatrix X,
                         int max_iter = 1000,
                         double conv = 1e-6,
                         double eps = 0.0,
                         bool bias_correction = false,
                         std::string center = "items",
                         double max_update = 1.5,
                         bool verbose = false,
                         bool estimatewle = false,
                         double wle_adj = 1e-8)
{
  const int N = X.nrow();
  const int I = X.ncol();
  if (I == 0)
    stop("Number of items must be > 0.");

  // Sanity check: only 0/1/NA
  for (int i = 0; i < N; ++i)
  {
    for (int k = 0; k < I; ++k)
    {
      double v = X(i, k);
      if (NumericMatrix::is_na(v))
        continue;
      if (!(v == 0.0 || v == 1.0))
        Rcpp::stop("X must contain only 0, 1, or NA (found %.8g at row %d, col %d).",
                   v, i + 1, k + 1);
    }
  }

  // Pre-compute row/col statistics
  IntegerVector row_obs(N, 0), col_obs(I, 0);
  NumericVector row_sum(N, 0.0), col_sum(I, 0.0);
  LogicalVector is_extreme(N, false);

  for (int p = 0; p < N; ++p)
  {
    for (int i = 0; i < I; ++i)
    {
      if (!NumericMatrix::is_na(X(p, i)))
      {
        row_obs[p]++;
        col_obs[i]++;
        double val = X(p, i);
        row_sum[p] += val;
        col_sum[i] += val;
      }
    }

    if (row_obs[p] > 0 && eps <= 0.0)
    {
      is_extreme[p] = (row_sum[p] <= 0.0 || row_sum[p] >= row_obs[p]);
    }
  }

  // Initialize theta and beta
  NumericVector theta(N, 0.0), beta(I, 0.0);

  for (int p = 0; p < N; ++p)
  {
    if (row_obs[p] > 0)
    {
      double prop = (row_sum[p] + 0.5) / (row_obs[p] + 1.0);
      prop = std::min(std::max(prop, 1e-12), 1.0 - 1e-12);
      theta[p] = std::log(prop / (1.0 - prop));
    }
  }

  for (int i = 0; i < I; ++i)
  {
    if (col_obs[i] > 0)
    {
      double prop = (col_sum[i] + 0.5) / (col_obs[i] + 1.0);
      prop = std::min(std::max(prop, 1e-12), 1.0 - 1e-12);
      beta[i] = -std::log(prop / (1.0 - prop));
    }
  }

  // Centering functions
  auto center_items = [&]()
  {
    double mean_beta = 0.0;
    for (int i = 0; i < I; ++i)
      mean_beta += beta[i];
    mean_beta /= (double)I;
    for (int i = 0; i < I; ++i)
      beta[i] -= mean_beta;
    for (int p = 0; p < N; ++p)
      theta[p] += mean_beta;
  };

  auto center_persons = [&]()
  {
    double mean_theta = 0.0;
    int cnt = 0;
    for (int p = 0; p < N; ++p)
    {
      if (!is_extreme[p])
      {
        mean_theta += theta[p];
        cnt++;
      }
    }
    if (cnt > 0)
    {
      mean_theta /= (double)cnt;
      for (int p = 0; p < N; ++p)
        theta[p] -= mean_theta;
      for (int i = 0; i < I; ++i)
        beta[i] += mean_theta;
    }
  };

  int iter = 0;
  String converged = "FALSE";

  NumericVector theta_grad(N), theta_info(N);
  NumericVector beta_grad(I), beta_info(I);

  if (center == "items")
    center_items();
  else if (center == "persons")
    center_persons();

  NumericVector theta_old = clone(theta);
  NumericVector beta_old = clone(beta);

  // Main iteration loop
  while (iter < max_iter)
  {
    // Update PERSONS
    std::fill(theta_grad.begin(), theta_grad.end(), 0.0);
    std::fill(theta_info.begin(), theta_info.end(), 0.0);

    for (int p = 0; p < N; ++p)
    {
      if (is_extreme[p])
        continue;

      for (int i = 0; i < I; ++i)
      {
        double x = X(p, i);
        if (NumericMatrix::is_na(x))
          continue;

        double z = theta[p] - beta[i];
        double P = logistic(z);
        double W = P * (1.0 - P);

        theta_grad[p] += x - P;
        theta_info[p] += W;
      }
    }

    for (int p = 0; p < N; ++p)
    {
      if (is_extreme[p])
        continue;
      if (theta_info[p] < 1e-12)
        theta_info[p] = 1.0;
      double step = theta_grad[p] / theta_info[p];
      if (step > max_update)
        step = max_update;
      else if (step < -max_update)
        step = -max_update;

      theta[p] += step;
    }

    // Update ITEMS
    std::fill(beta_grad.begin(), beta_grad.end(), 0.0);
    std::fill(beta_info.begin(), beta_info.end(), 0.0);

    for (int i = 0; i < I; ++i)
    {
      for (int p = 0; p < N; ++p)
      {
        if (is_extreme[p])
          continue;

        double x = X(p, i);
        if (NumericMatrix::is_na(x))
          continue;

        double z = theta[p] - beta[i];
        double P = logistic(z);
        double W = P * (1.0 - P);

        beta_grad[i] += P - x;
        beta_info[i] += W;
      }
    }

    for (int i = 0; i < I; ++i)
    {
      if (beta_info[i] < 1e-12)
        beta_info[i] = 1.0;
      double step = beta_grad[i] / beta_info[i];
      if (step > max_update)
        step = max_update;
      else if (step < -max_update)
        step = -max_update;

      beta[i] += step;
    }

    // Center after both updates
    if (center == "items")
      center_items();
    else if (center == "persons")
      center_persons();

    // Check convergence
    double max_change_items = 0.0;
    for (int i = 0; i < I; ++i)
    {
      double change = std::fabs(beta[i] - beta_old[i]);
      if (change > max_change_items)
        max_change_items = change;
    }

    double max_change_persons = 0.0;
    for (int p = 0; p < N; ++p)
    {
      if (is_extreme[p])
        continue;
      double change = std::fabs(theta[p] - theta_old[p]);
      if (change > max_change_persons)
        max_change_persons = change;
    }

    double max_change = std::max(max_change_items, max_change_persons);

    if (verbose && (iter < 5 || iter % 10 == 0))
    {
      Rprintf("Iteration %4d: max_items=%.8f, max_persons=%.8f\n",
              iter, max_change_items, max_change_persons);
      R_FlushConsole();
    }

    if (max_change < conv)
    {
      converged = "TRUE";
      if (verbose)
        Rprintf("Converged: max_items=%.8f, max_persons=%.8f\n",
                max_change_items, max_change_persons);
      break;
    }

    theta_old = clone(theta);
    beta_old = clone(beta);

    iter++;
  }


  // Optional WLE estimation
  NumericVector theta_wle(N, NA_REAL);
  if (estimatewle)
  {
    List w = estimate_wle(X, beta, max_iter, 1e-10, NA_REAL, NA_REAL, wle_adj);
    theta_wle = w["wle"];
  }

  // Step 1: Apply bias correction to beta
  if (bias_correction && I > 1)
  {
    double factor = (double)(I - 1) / (double)I;
    for (int i = 0; i < I; ++i)
    {
      beta[i] *= factor;
    }
  }

  // Step 2: Apply final centering constraint (AFTER bias correction)
  if (center == "items")
    center_items();
  else if (center == "persons")
    center_persons();

  // Step 3: THEN assign ±Inf to extreme persons
  if (eps <= 0.0)
  {
    for (int p = 0; p < N; ++p)
    {
      if (row_obs[p] > 0)
      {
        if (row_sum[p] <= 0.0)
          theta[p] = R_NegInf;
        else if (row_sum[p] >= row_obs[p])
          theta[p] = R_PosInf;
      }
    }
  }

  return List::create(
      _["data"] = X,
      _["theta"] = theta,
      _["beta"] = beta,
      _["iterations"] = iter,
      _["converged"] = converged,
      _["bias_correction"] = bias_correction,
      _["center"] = center,
      _["wle_estimate"] = theta_wle);
}

/* -----------------------------------------------------------------------
Exported wrappers
------------------------------------------------------------------------- */
// [[Rcpp::export]]
Rcpp::List estimate_wle(SEXP X_, Rcpp::NumericVector beta,
                        int max_iter = 100, double tol = 1e-10,
                        double lower_ext = NA_REAL, double upper_ext = NA_REAL,
                        double wle_adj = 1e-8)
{
  try
  {
    Rcpp::NumericMatrix X = require_real_matrix(X_);
    return estimate_wle(X, beta, max_iter, tol, lower_ext, upper_ext, wle_adj);
  }
  catch (std::exception &e)
  {
    Rcpp::stop(e.what());
  }
  catch (...)
  {
    Rcpp::stop("estimate_wle: unknown error.");
  }
}

// [[Rcpp::export]]
Rcpp::List estimate_jmle(SEXP X_,
                         int max_iter = 1000,
                         double conv = 1e-6,
                         double eps = 0.0,
                         SEXP bias_correction_ = R_NilValue,
                         std::string center = "items",
                         double max_update = 1.5,
                         bool verbose = false,
                         bool estimatewle = false,
                         double wle_adj = 1e-8)
{
  try
  {
    Rcpp::NumericMatrix X = require_real_matrix(X_);
    if (bias_correction_ == R_NilValue)
    {
      Rcpp::stop("bias_correction must be provided.");
    }
    if (!Rf_isLogical(bias_correction_) || Rf_xlength(bias_correction_) != 1)
      Rcpp::stop("bias_correction must be a single logical value (TRUE/FALSE).");
    bool bias_correction = Rcpp::as<bool>(bias_correction_);
    return estimate_jmle(X, max_iter, conv, eps,
                         bias_correction,
                         center, max_update,
                         verbose, estimatewle, wle_adj);
  }
  catch (std::exception &e)
  {
    Rcpp::stop(e.what());
  }
  catch (...)
  {
    Rcpp::stop("estimate_jmle: unknown error.");
  }
}
